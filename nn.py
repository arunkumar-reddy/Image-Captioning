...
def convolution_no_bias(feats,filter_h,filter_w,output_depth,stride_h,stride_w,name,init_w='he',stddev=0.01,padding='SAME',group_id=0):
	...
	convolve = lambda i,k: tf.nn.conv2d(i,k,[1,stride_h,stride_w,1],padding=padding);
	with tf.variable_scope(name) as scope:
		weights = weight('weights',[filter_h,filter_w,input_depth,otuput_depth],init=init_w,stddev=stddev,group_id=group_id);
		output = convolve(feats,weights);
	return output;
...
def batch_norm(weights,name,train,bn=True,nonlinearity='relu'):
	if bn:
		weights = normalise(weights,name,train);
	weights = nonlinear(weights,nonlinearity);
	return weights;
